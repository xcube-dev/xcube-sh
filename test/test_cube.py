...
cube_config_t1_none = CubeConfig(
    dataset_name="S2L1C",
    band_names=["B04"],
    bbox=(10.00, 54.27, 11.00, 54.60),
    spatial_res=0.00018,
...
cube_config_with_crs = CubeConfig(
    dataset_name="S2L1C",
    band_names=["B01"],
    tile_size=(512, 512),
...
cube_config_S2L2A_1D = CubeConfig(
    dataset_name="S2L2A",
    band_names=["B03", "B08", "CLM"],
    bbox=(2894267.8988124575, 9262943.968658403, 2899443.8488556934, 9268505.554239485),
    crs="http://www.opengis.net/def/crs/EPSG/0/3857",
...
cube_config_S2L2A_WGS84 = CubeConfig(
    dataset_name="S2L2A",
    band_names=["B03", "B08", "CLM"],
...
cube_config_S2L2A_WGS84_1D = CubeConfig(
    dataset_name="S2L2A",
    band_names=["B03", "B08", "CLM"],
    bbox=(25.99965089839723, 63.65600798545179, 26.046183630114623, 63.67816348259773),
    spatial_res=0.0001,
...
class CubeTest(unittest.TestCase):
    ...
    def test_time_max_none(self):
        ...
        self.assertIsInstance(cube, xr.Dataset)
        self.assertEqual(
            np.datetime64("today", "D"), np.datetime64(cube.time.values[-1], "D")
        )
        self.assert_has_zarr_store(cube)
    ...
    def test_time_none(self):
        ...
        self.assertIsInstance(cube, xr.Dataset)
        self.assertEqual(
            np.datetime64("2021-02-16"), np.datetime64(cube.time.values[-1], "D")
        )
        self.assertEqual(
            np.datetime64("2015-06-27"), np.datetime64(cube.time.values[0], "D")
        )
        self.assert_has_zarr_store(cube)

    def assert_has_zarr_store(self, cube):
        if hasattr(cube, "zarr_store"):
            self.assertIsInstance(cube.zarr_store.get(), collections.abc.Mapping)
