...
class CubeConfigTest(unittest.TestCase):
    ...
    def test_time_none(self):
        ...
        self.assertEqual(
            np.datetime64("today", "s"), np.datetime64(config.time_range[1], "s")
        )
        ...
        self.assertEqual(
            np.datetime64("1970-01-01", "s"), np.datetime64(config.time_range[0], "s")
        )
    ...
    def test_crs(self):
        ...
        config = CubeConfig(
            crs="http://www.opengis.net/def/crs/EPSG/0/4326", **we_do_not_care
        )
        self.assertEqual("WGS84", config.crs)
        ...
        config = CubeConfig(
            crs="http://www.opengis.net/def/crs/OGC/1.3/CRS84", **we_do_not_care
        )
        self.assertEqual("CRS84", config.crs)
        ...
        config = CubeConfig(
            crs="http://www.opengis.net/def/crs/EPSG/0/3857", **we_do_not_care
        )
        self.assertEqual("EPSG:3857", config.crs)
