...
@click.option(
    "--res", "-r", "spatial_res", help="Spatial resolution in degrees.", type=float
)
@click.option(
    "--crs",
...
@click.option(
    ...
    "-o",
    "output_path",
    help=f"Output ZARR directory. " f'Defaults to "{DEFAULT_GEN_OUTPUT_PATH}".',
)
@click.option(
    "--4d",
    "four_d",
...
def gen(
    ...
    if "datastore_id" in input_config_dict:
        ...
        if datastore_id != "sentinelhub":
            warnings.warn(
                f"Unknown datastore_id={datastore_id!r} "
    ...
    _overwrite_config_params(output_config_dict, path=output_path)
    # TODO: validate output_config_dict

    cube_config = CubeConfig.from_dict(
        cube_config_dict, exception_type=click.ClickException
    )
    ...
    if not _is_bucket_url(output_path) and os.path.exists(output_path):
        raise click.ClickException(
            f"Output {output_path} " f"already exists. Move it away first."
        )
    ...
    print(f"Cube written to {output_path}, " f"took {'%.2f' % cm.duration} seconds.")
...
def req(output_path: str, is_s3_config: bool):
    """
    ...
    if is_s3_config:
        ...
        output_config = dict(path=DEFAULT_GEN_OUTPUT_PATH)
    config_dict = dict(
        input_config=input_config, cube_config=cube_config, output_config=output_config
    )
    if output_path:
    ...
    if not datasets:
        ...
        for dataset_name in datasets:
            ...
            for band in bands:
                ...
                band_dict.update(
                    sentinel_hub.METADATA.dataset_band(
                        dataset_name, band_name, default={}
                    )
                )
...
def _load_request(request_file: Optional[str]) -> Dict[str, Any]:
    ...
    if request_file and not os.path.exists(request_file):
        raise click.ClickException(f"Configuration file " f"{request_file} not found.")
    ...
    try:
        ...
        raise click.ClickException(
            f"Error loading configuration " f"file {request_file}: {e}"
        )
