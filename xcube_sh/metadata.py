...
class SentinelHubMetadata:
    ...
    def collection_datasets(self) -> Dict:
        return {
            v["collection_name"]: dict(**v, dataset_name=k)
    ...
    def dataset_processing_level(
        self, dataset_name: str, default=None
    ) -> Optional[str]:
        dataset = self._dataset_direct(dataset_name)
    ...
    def dataset_band(
        self, dataset_name: str, band_name: str, default=None
    ) -> Optional[Dict]:
        band = self._dataset_band_direct(dataset_name, band_name)
        return dict(band) if band else default

    def dataset_band_sample_type(
        self, dataset_name: str, band_name: str, default="FLOAT32"
    ) -> str:
        band = self._dataset_band_direct(dataset_name, band_name)
        return band.get("sample_type", default) if band else default

    def dataset_band_fill_value(
        self, dataset_name: str, band_name: str, default=None
